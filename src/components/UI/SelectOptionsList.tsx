...
const SelectOptionsList = ({currentValue, setCurrentValue, variants}: selectOptionsInterface) => {
    ...
    return (
        <div className={'w-32 h-8 rounded-full flex items-center justify-center border-white border-[1px]'}>
            <div className={'w-full relative text-white font-semibold text-center'} onClick={()=>{setIsOpen(!isOpen)}}>
                ...
                {isOpen ? <div className={'duration-300 transition-transform duration-300 absolute text-left pl-3 p-1 bg-white text-black z-50 w-32 left-0 top-9 rounded-xl' +toggleAnimation()}>{variants.map((item) => {
                    if(item!=currentValue){
                        return <p key={item} onClick={()=>{setCurrentValue(item)}} className={'my-2 hover:bg-black hover:bg-opacity-5 cursor-pointer border-b-2'}>{item}</p>
                    }
                })}</div> :null}
            </div>
        </div>
